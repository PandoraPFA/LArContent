...
{
    // Check the input PFO lists exist
    ...
    const StatusCode statusCodeCosmic(PandoraContentApi::GetList(*this, m_inputCosmicRayPfoListName, pCosmicPfoList));

    if (STATUS_CODE_SUCCESS != statusCodeCosmic)
    {
        std::cout << "CosmicRayShowerMatchingAlgorithm: Cosmic pfo list unavailable " << std::endl;
        return STATUS_CODE_SUCCESS;
    }
...
StatusCode CosmicRayShowerMatchingAlgorithm::CosmicRayShowerMatching(ctopmap_t &clusterToPfoMap, ptocmultimap_t &pfoAssociatedClusterMap) const
{
    ...
    PfoVector cosmicPfoVector(pCosmicPfoList->begin(), pCosmicPfoList->end());
    std::sort(cosmicPfoVector.begin(), cosmicPfoVector.end(), CosmicRayShowerMatchingAlgorithm::SortPfosByNHits);

    for (PfoVector::const_iterator iterPfo = cosmicPfoVector.begin(), iterPfoEnd = cosmicPfoVector.end(); iterPfo != iterPfoEnd; ++iterPfo)
    {
