...
StatusCode TransverseAssociationAlgorithm::Run()
{
...
bool TransverseAssociationAlgorithm::IsExtremalCluster( const bool isForward, const Cluster *const pCurrentCluster,  const Cluster *const pTestCluster ) const
{
    ...
    if ( isForward ) return ( testMaxX > currentMaxX ); 
    else             return ( testMinX < currentMinX ); 
}
...
void TransverseAssociationAlgorithm::GetSeedClusters( const pandora::ClusterVector &shortVector, const pandora::ClusterVector &longVector, pandora::ClusterVector &seedVector,  pandora::ClusterVector &nonSeedVector ) const
{
    // clear vectors
    seedVector.clear();
    nonSeedVector.clear();
...
void TransverseAssociationAlgorithm::GetTransverseAssociatedClusters( const Cluster *const inputCluster, const ClusterVector &inputClusterVector, ClusterVector &outputClusterVector ) const
{  
    // clear vector 
...
void TransverseAssociationAlgorithm::FillTransverseClusterMap( const ClusterVector &seedClusters, const ClusterVector &transverseClusters, LArTransverseClusterMap &transverseClusterMap ) const
{ 
...
void TransverseAssociationAlgorithm::FillClusterAssociationMap(const LArTransverseClusterMap &transverseClusterMap, ClusterAssociationMap &clusterAssociationMap) const
{
        ...
        for( unsigned int n = 0; n < transCluster.GetNumClusters(); ++n )
	{
            ...
            if( this->IsExtremalCluster( true,  pCentralCluster, pCluster ) == true 
             && this->IsExtremalCluster( false, pCentralCluster, pCluster ) == false )
	    {
            ...
            if( this->IsExtremalCluster( false, pCentralCluster, pCluster ) == true 
             && this->IsExtremalCluster( true,  pCentralCluster, pCluster ) == false )
	    {
    ...
    for ( LArTransverseClusterMap::const_iterator iter1 = transverseClusterMap.begin(), iterEnd1 = transverseClusterMap.end(); iter1 != iterEnd1; ++iter1 )
    {
        ...
        for ( LArTransverseClusterMap::const_iterator iter2 = transverseClusterMap.begin(), iterEnd2 = transverseClusterMap.end(); iter2 != iterEnd2; ++iter2 )
        {
            ...
            if( this->IsExtremalCluster( true,  pInnerCluster, pOuterCluster ) == true 
	     && this->IsExtremalCluster( false, pOuterCluster, pInnerCluster ) == true )
	    {
        ...
        for ( ClusterList::iterator iter2 = iter1->second.begin(), iterEnd2 = iter1->second.end(); iter2 != iterEnd2; ++iter2 )
	{
            ...
            for ( ClusterList::iterator iter3 = iter1->second.begin(), iterEnd3 = iter1->second.end(); iter3 != iterEnd3; ++iter3 )
	    {
	        Cluster* pMiddleCluster = *iter3;
                ...
                if ( pMiddleCluster == pOuterCluster ) continue;

                if ( forwardMergeMap[pMiddleCluster].find(pOuterCluster) != forwardMergeMap[pMiddleCluster].end() ) 
		{
		    isNeighbouringCluster = false;  break;
		}
	    }

            if ( isNeighbouringCluster )
	    {
	        if ( clusterAssociationMap[pInnerCluster].m_forwardAssociations.find(pOuterCluster)
	          == clusterAssociationMap[pInnerCluster].m_forwardAssociations.end() )
                    clusterAssociationMap[pInnerCluster].m_forwardAssociations.insert(pOuterCluster);

		if ( clusterAssociationMap[pOuterCluster].m_backwardAssociations.find(pInnerCluster)
		  == clusterAssociationMap[pOuterCluster].m_backwardAssociations.end() )
        ...
        for ( ClusterList::iterator iter2 = iter1->second.begin(), iterEnd2 = iter1->second.end(); iter2 != iterEnd2; ++iter2 )
	{
            ...
            for ( ClusterList::iterator iter3 = iter1->second.begin(), iterEnd3 = iter1->second.end(); iter3 != iterEnd3; ++iter3 )
	    {
	        Cluster* pMiddleCluster = *iter3;
                ...
                if ( pMiddleCluster == pInnerCluster ) continue;
    ...
    while( carryOn )
    {
        ...
        for ( ClusterAssociationMap::iterator iter1 = clusterAssociationMap.begin(), iterEnd1 = clusterAssociationMap.end(); iter1 != iterEnd1; ++iter1 )
        {
            ...
            if ( iter1->second.m_backwardAssociations.size() > 0
	      && iter1->second.m_forwardAssociations.size() > 0 ) 
                continue;
            ...
            if ( iter1->second.m_forwardAssociations.size() == 1 )
	    {
	        ...
	        { 
                    ...
                    if ( iter2->second.m_forwardAssociations.size() == 0 )
	            {
            ...
            if ( iter1->second.m_backwardAssociations.size() == 1 )
	    {
                ...
                if ( pBackwardCluster->GetNCaloHits() < m_clusterCaloHits )
	        {
                    ...
                    if ( iter2->second.m_backwardAssociations.size() == 0 )
	            {
...
{    
    ...
    const float aveX1( 0.5 * ( pCluster1->GetCentroid( pCluster1->GetInnerPseudoLayer() ).GetX()
		             + pCluster1->GetCentroid( pCluster1->GetOuterPseudoLayer() ).GetX() ) );

    const float aveZ1( 0.5 * ( pCluster1->GetCentroid( pCluster1->GetInnerPseudoLayer() ).GetZ()
		             + pCluster1->GetCentroid( pCluster1->GetOuterPseudoLayer() ).GetZ() ) );

    const float aveX2( 0.5 * ( pCluster2->GetCentroid( pCluster2->GetInnerPseudoLayer() ).GetX()
		             + pCluster2->GetCentroid( pCluster2->GetOuterPseudoLayer() ).GetX() ) );

    const float aveZ2( 0.5 * ( pCluster2->GetCentroid( pCluster2->GetInnerPseudoLayer() ).GetZ()
