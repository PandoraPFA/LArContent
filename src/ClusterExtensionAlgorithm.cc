/**
 *  @file   ClusterExtensionAlgorithm.cc
 * 
...
{  
    ...
    const float clusterOccupancyI = LArParticleId::LArLayerOccupancy(pClusterI);
    const float clusterOccupancyJ = LArParticleId::LArLayerOccupancy(pClusterJ);

    if ( LArParticleId::LArLayerOccupancy(pClusterI, pClusterJ) < 0.75 ) return;
