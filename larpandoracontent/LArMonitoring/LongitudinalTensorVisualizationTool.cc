/**
 *  @file   larpandoracontent/LArMonitoring/LongitudinalTensorVisualizationTool.cc
 *
 *  @brief  Implementation of the longitudinal tensor visualization tool class.
 *
 *  $Log: $
 */
...
LongitudinalTensorVisualizationTool::LongitudinalTensorVisualizationTool() :
...
bool LongitudinalTensorVisualizationTool::Run(ThreeViewLongitudinalTracksAlgorithm *const pAlgorithm, TensorType &overlapTensor)
{
    ...
    for (const Cluster *const pKeyCluster : sortedKeyClusters)
    {
        ...
        for (TensorType::ElementList::const_iterator eIter = elementList.begin(); eIter != elementList.end(); ++eIter)
        {
            ...
            std::cout << " Element " << counter++ << ": MatchedFraction " << eIter->GetOverlapResult().GetMatchedFraction()
                      << ", MatchedSamplingPoints " << eIter->GetOverlapResult().GetNMatchedSamplingPoints() << ", Chi2 "
                      << eIter->GetOverlapResult().GetReducedChi2() << std::endl;
...
StatusCode LongitudinalTensorVisualizationTool::ReadSettings(const TiXmlHandle xmlHandle)
{
