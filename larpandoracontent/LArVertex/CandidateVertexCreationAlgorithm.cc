...
StatusCode CandidateVertexCreationAlgorithm::Run()
{
    try
    {
        ...
        StatusCode status{PandoraContentApi::GetCurrentList(*this, pVertexList)};
...
void CandidateVertexCreationAlgorithm::AddInputVertices() const
{
    ...
    { // ATTN - No guarantee the list has been initialised, but silent failure here is ok
        StatusCode status{PandoraContentApi::GetList(*this, m_inputVertexListName, pInputVertexList)};
        if (status != STATUS_CODE_SUCCESS || !pInputVertexList || pInputVertexList->empty())
            return;
