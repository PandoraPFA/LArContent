    ...
    const VertexSelectionBaseAlgorithm::SlidingFitDataList &slidingFitDataList) const
{
    ...
    for (const VertexSelectionBaseAlgorithm::SlidingFitData &slidingFitData : slidingFitDataList)
    {
        ...
        this->IncrementEnergyKickParameters(pCluster, clusterDisplacement, clusterDirection, totEnergyKick, totEnergy, totHitKick, totHits, clusterEnergy);
    }
...
void EnergyKickFeatureTool::IncrementEnergyKickParameters(const Cluster *const pCluster, const CartesianVector &clusterDisplacement,
    const CartesianVector &clusterDirection, float &totEnergyKick, float &totEnergy, float &totHitKick, unsigned int &totHits, const float clusterEnergy) const
{
    ...
    totEnergyKick += clusterEnergy * (impactParameter + m_xOffset) / (displacement + m_rOffset);
