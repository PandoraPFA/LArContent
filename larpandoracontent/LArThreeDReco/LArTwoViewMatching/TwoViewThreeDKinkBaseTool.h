/**
 *  @file
 * larpandoracontent/LArThreeDReco/LArTwoViewMatching/TwoViewThreeDKinkBaseTool.h
 *
...
protected:
   ...
   */
    ...
    /**
   *  @brief  Get modification objects for a specific elements of the matrix,
   * identifying required splits and merges for clusters
   *
   ...
   */
    virtual void GetIteratorListModifications(
        TwoViewTransverseTracksAlgorithm *const pAlgorithm, const IteratorList &iteratorList, ModificationList &modificationList) const = 0;
   ...
   */
    ...
    bool m_majorityRulesMode;               ///< Whether to run in majority rules mode (always
                                            ///< split overshoots, always merge undershoots)
    float m_minXOverlapFraction;            ///< The min x overlap fraction value for
                                            ///< particle creation
    float m_minMatchingScore;               ///< The min global matching score for particle
                                            ///< creation
    float m_minLocallyMatchedFraction;      ///< The min locally matched fraction for
                                            ///< particle creation
    float m_minLongitudinalImpactParameter; ///< The min longitudinal impact
                                            ///< parameter for connecting
                                            ///< accompanying clusters
    int m_nLayersForKinkSearch;             ///< The number of sliding fit layers to step in
                                            ///< the kink search
    float m_additionalXStepForKinkSearch;   ///< An additional (safety) step to
                                            ///< tack-on when choosing x sampling
                                            ///< points
