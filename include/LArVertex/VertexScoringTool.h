...
private:
     ...
     */
    void InitializeKDTrees(const pandora::Algorithm *const pAlgorithm, HitKDTree2D &kdTreeU, HitKDTree2D &kdTreeV, HitKDTree2D &kdTreeW) const;
     ...
     */
    void InitializeKDTree(const pandora::Algorithm *const pAlgorithm, const std::string &caloHitListName, HitKDTree2D &kdTree) const;
    ...
    bool            m_beamMode;                     ///< Whether to ScoreVertices in beam mode, assuming neutrinos travel in positive z-direction
...
inline pandora::AlgorithmTool *VertexScoringTool::Factory::CreateAlgorithmTool() const
{
    return new VertexScoringTool();
}

//------------------------------------------------------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------------------------------------------------------

inline VertexScoringTool::VertexScore::VertexScore(const pandora::Vertex *const pVertex, const float score) :
    m_pVertex(pVertex),
    m_score(score)
{
}
...
inline const pandora::Vertex *VertexScoringTool::VertexScore::GetVertex() const
{
    return m_pVertex;
}
...
inline bool VertexScoringTool::VertexScore::operator< (const VertexScore &rhs) const
{
...
inline float VertexScoringTool::BeamConstants::GetDecayConstant() const
{
...
inline void VertexScoringTool::BeamConstants::SetConstants(const float minZCoordinate, const float decayConstant)
{
...
inline VertexScoringTool::KernelEstimate::KernelEstimate(const float sigma) :
    m_sigma(sigma)
{
...
inline const VertexScoringTool::KernelEstimate::ContributionList &VertexScoringTool::KernelEstimate::GetContributionList() const
{
...
inline float VertexScoringTool::KernelEstimate::GetSigma() const
{
    return m_sigma;
}
...
#endif // #ifndef LAR_VERTEX_SCORING_TOOL_H
